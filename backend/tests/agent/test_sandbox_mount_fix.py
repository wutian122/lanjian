...
class TestCommandNeedsProjectMount:
    ...
    def test_needs_mount_heredoc(self):
        """根因1: heredoc 写入命令（不含 /workspace/ 引用）返回 False（不挂载）"""
        cmd = "cat > /tmp/poc.py << 'EOF'\nprint('x')\nEOF\npython3 /tmp/poc.py"
        assert SandboxTool._command_needs_project_mount(cmd) is False

    def test_needs_mount_heredoc_with_workspace_ref(self):
        """根因1修复: heredoc 写入的脚本内容含 /workspace/src/ 引用时应返回 True（需挂载）

        实际 Verification Agent 生成的 PoC 命令均为 heredoc 格式，脚本内引用
        /workspace/src/{file_path}。修复前 heredoc 短路导致不挂载，PoC 找不到文件。
        """
        cmd = (
            "cat > /tmp/poc_0.py << 'POC_EOF'\n"
            "import os\n"
            "src = '/workspace/src/auth.py'\n"
            "print(open(src).read())\n"
            "POC_EOF\n"
