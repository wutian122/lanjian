...
# ---------- Scenario 1 归因：Final Answer 无 findings 时日志含截断线索 ----------
...
@pytest.mark.asyncio
async def test_final_answer_no_findings_log_attributes_truncation(monkeypatch, caplog):
    """Final Answer 无 findings 键且上轮被截断 → 容器日志归因'疑似 max_tokens 截断'。"""
    ...
    no_findings_logs = [
        ...
        if "Final Answer has no 'findings' key" in r.message
    ]
    assert no_findings_logs, "应走到 Final Answer 无 findings 告警分支"
    assert any("疑似 max_tokens 截断" in m for m in no_findings_logs), no_findings_logs


@pytest.mark.asyncio
async def test_final_answer_no_findings_log_no_truncation_hint_when_clean(monkeypatch, caplog):
    """对照：未发生截断时，无 findings 日志不得包含'疑似截断'线索。"""
    agent = _make_analysis_agent(monkeypatch)
    ...
    assert no_findings_logs, "应走到 Final Answer 无 findings 告警分支"
    assert all("疑似" not in m for m in no_findings_logs), no_findings_logs
