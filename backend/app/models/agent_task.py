...
class AgentTaskStatus:
    ...
    COMPLETED_WITH_GAPS = "completed_with_gaps"  # 已完成但覆盖率不足（安全阀放行）
    FAILED = "failed"             # 失败
...
class AgentTask(Base):
    ...
    last_checkpoint_id = Column(String(36), nullable=True)
    ...
    verified_count = Column(Integer, default=0)  # 已验证数（仅 confirmed 动态复现）
    static_confirmed_count = Column(Integer, default=0)  # 代码推理确认数（未动态复现）
    ...
    verification_coverage = Column(Float, nullable=True)  # 验证覆盖率 0.0-1.0
    observations = Column(JSON, nullable=True)  # 被校验 gate 拒绝的观察项
    ...
    def progress_percentage(self) -> float:
        """计算进度百分比"""
        if self.status in [AgentTaskStatus.COMPLETED, AgentTaskStatus.COMPLETED_WITH_GAPS]:
            return 100.0
        ...
        for phase, weight in phase_weights.items():
            if phase == self.current_phase:
                ...
                elif phase == AgentTaskPhase.VERIFICATION:
                    # 按 tool_calls_count 相对 findings_count 估算进度
                    if self.findings_count > 0:
                        ratio = min(self.tool_calls_count / max(self.findings_count * 3, 1), 0.95)
                        completed_weight += weight * ratio
                    else:
                        completed_weight += weight * 0.3
                elif phase == AgentTaskPhase.REPORTING:
                    completed_weight += weight * 0.8
                else:
...
class VerificationStatus:
    """Strict AI verification outcome."""
    CONFIRMED = "confirmed"
    STATIC_CONFIRMED = "static_confirmed"  # 代码推理确认（未动态复现）
    FALSE_POSITIVE = "false_positive"
    NOT_REPRODUCIBLE = "not_reproducible"
    NEEDS_CONTEXT = "needs_context"
    ALL = {CONFIRMED, STATIC_CONFIRMED, FALSE_POSITIVE, NOT_REPRODUCIBLE, NEEDS_CONTEXT}
...
class AgentFinding(Base):
    ...
    verification_status = Column(String(32), default=VerificationStatus.NEEDS_CONTEXT, index=True)
