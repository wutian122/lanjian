...
class BaseLLMAdapter(ABC):
    ...
    @staticmethod
    def _detect_provider_error(error_str: str) -> Optional[str]:
        """检测 AI 服务商侧特定故障并返回友好提示

        用于识别如讯飞 one-api 网关鉴权失败等服务商侧问题。
        """
    ...
    def handle_error(self, error: Any, context: str = "", api_response: str = None) -> None:
        ...
        if "超时" in message or "timeout" in message.lower():
            ...
            # 检测 AI 服务商侧特定故障（如讯飞 one-api 鉴权异常）
            api_err = api_response or message
            provider_msg = self._detect_provider_error(api_err)
            if provider_msg:
