...
from app.core.scan_constants import TEXT_EXTENSIONS
...
# TEXT_EXTENSIONS 统一来自 app.core.scan_constants（单一真相源）
...
class ChromaVectorStore(VectorStore):
    ...
    async def update_collection_metadata(self, updates: Dict[str, Any]):
        ...
        try:
            ...
            # R2: hnsw:space 是 collection 创建时的不可变 HNSW 参数，传入 modify 会触发
            # "Changing the distance function of a collection once it is created is not supported"
            # 告警。剥离后再 modify。
            modify_metadata = {k: v for k, v in current_metadata.items() if k != "hnsw:space"}
...
class CodeIndexer:
    ...
    async def _full_index(
        ...
        # [Fix-Q1] If no new chunks this run, read total from collection
        if all_chunks:
            progress.indexed_chunks = len(all_chunks)
        else:
            try:
                progress.indexed_chunks = await self.vector_store.get_count()
            except Exception:
    ...
    async def _incremental_index(
        ...
        # [Fix-Q1] pre-assign indexed_chunks before embedding to avoid 0 on timeout
        # [Fix-Q1] If no new chunks this run, read total from collection
        if all_chunks:
            progress.indexed_chunks = len(all_chunks)
        else:
    ...
    async def _index_chunks(
        ...
        embeddings = await self.embedding_service.embed_batch(
            texts,
            batch_size=200,  # SiliconFlow bge-m3 supports larger batches 
