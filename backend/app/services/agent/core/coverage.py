...
class CoverageReport:
    ...
    def is_sufficient(self) -> bool:
        """覆盖率是否达标：covered(含浅覆盖) >= 6 且 D1/D2/D3 全部深度 covered

        修复：浅覆盖（grep 搜过的维度）计入 covered_count，阈值 8→6。
        避免普通项目因不可能有 8 类真实漏洞而每次触发补充审计。
        核心三角 D1/D2/D3 仍要求深度覆盖（真实发现）。
        """
        if self.covered_count < 6:
            return False
...
class CoverageMatrix:
    ...
    def to_report(self) -> CoverageReport:
        ...
        修复：covered_count 计入浅覆盖（shallow），使"审查过但无漏洞"的维度
        也算作已覆盖。deep_covered_count 单独保留深度覆盖数供参考。
        """
        deep = sum(1 for d in self._matrix.values() if d["status"] == "covered")
