...
class AgentGraphController:
    ...
    def stop_task_agents(self, task_id: str) -> dict[str, Any]:
        """
        C1: 只停止指定任务的 Agent 子树，不影响其他并发任务。

        Args:
            task_id: Agent 审计任务 ID

        Returns:
            操作结果
        """
        agent_ids = agent_registry.get_task_agent_ids(task_id)
        if not agent_ids:
            return {
                "success": True,
                "stopped_count": 0,
                ...
                "stopped": [],
                "failed": [],
                "message": f"任务 {task_id} 没有已注册的 Agent",
            }
        ...
        for agent_id in agent_ids:
            node = agent_registry.get_agent_node(agent_id)
            if node and node.get("status") in ["completed", "failed", "stopped"]:
                continue
            result = self.stop_agent(agent_id)
            if result.get("success"):
                stopped.append(agent_id)
        ...
        return {
            ...
            "stopped": stopped,
            "failed": failed,
            "message": f"已向任务 {task_id} 的 {len(stopped)} 个 Agent 发送停止请求",
        }
...
def stop_task_agents(task_id: str) -> dict[str, Any]:
    """C1: 只停止指定任务的 Agent 子树（并发隔离）。"""
    return agent_graph_controller.stop_task_agents(task_id)
