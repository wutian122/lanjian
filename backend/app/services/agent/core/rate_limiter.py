...
class TokenBucketRateLimiter:
    ...
    def reconfigure(self, rate: float, burst: int) -> None:
        """Update limiter parameters without discarding the existing bucket state."""
        self._replenish()
        self.rate = rate
        self.burst = burst
        self.tokens = min(float(burst), self.tokens)
...
class RateLimiterRegistry:
    ...
    def get_or_create(
        ...
        if limiter.rate != rate or limiter.burst != burst:
...
def get_llm_rate_limiter() -> TokenBucketRateLimiter:
    """Get rate limiter for LLM calls from dynamic RPM config."""
    from app.services.agent.config import get_agent_config

    rpm = max(1, int(get_agent_config().llm_rate_per_minute or 60))
    rate = rpm / 60.0
    return get_rate_limiter("llm", rate=rate, burst=5)


def get_task_llm_rate_limiter(task_id: str, rpm: int = 60) -> TokenBucketRateLimiter:
    """Get task-scoped rate limiter for LLM calls.

    每个 Agent 审计任务拥有独立的 token bucket（按 task_id 命名），
    多任务并发时各自独立享用各自的 RPM，互不干扰、互不覆盖。
    """
    rpm = max(1, int(rpm or 60))
    rate = rpm / 60.0
    return get_rate_limiter(f"llm_{task_id}", rate=rate, burst=5)
