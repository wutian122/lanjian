...
_LANGUAGE_SINK_PATTERNS: dict[str, dict[str, list[str]]] = {
    ...
    "path_traversal": {
        ".java": [r"new\s+File\s*\(", r"Paths\.get", r"getResource\s*\(", "FileInputStream"],
        ".py": [r"os\.path\.join", r"\.\./", r"open\(", r"pathlib"],
        ".php": [r"file_get_contents", "readfile", r"include\s*\("],
        ".rb": ["File.read", "File.open"],
        "default": [r"os\.path\.join", r"\.\./", r"open\(", r"pathlib", "send_file", "send_from_directory"],
    },
    "command_injection": {
        ".java": [r"Runtime\.getRuntime\(\)\.exec", "ProcessBuilder", r"\bexec\s*\(", "getRuntime"],
        ".py": ["subprocess", r"os\.system", r"os\.popen", "shell=True", r"eval\s*\(", r"exec\s*\("],
        ".php": [r"system\s*\(", "shell_exec", r"exec\s*\(", "passthru"],
        "default": ["subprocess", r"os\.system", r"os\.popen", r"eval\s*\(", r"exec\s*\("],
    },
    "xxe": {
...
{f"特别注意 Analysis Agent 提到的关注点。" if handoff_context else ""}"""
    ...
    def _record_sandbox_attempt(
        ...
        # Task 4：VULNERABILITY_STATIC_ONLY（源码在案 sink 但无动态演示/密钥类无法
        # 动态确认）同属静态证据档——此前该标记零消费点，secret/deserialization/
        # xss-sink-only 的确定性 PoC 输出空转 not_reproducible（design 第 3 节：
        # "统一走既有 static_evidence 识别，终态上限 static_confirmed"）。
        static_evidence = (
            "vulnerability_confirmed(static)" in obs_lower
            or "vulnerability_static_only" in obs_lower
        )
        has_vuln_evidence = (
            not static_evidence
        ...
        cmd_templates = {
            ...
            'path_traversal': {
                'command': (
                    ...
                    f"    with open(src) as f: content = f.read()\n"
                    f"    print(f'Source: {{len(content)}} chars loaded')\n"
                    f"    # Step 1: 静态确认路径拼接 sink（文件读写 API/路径拼接 pattern）\n"
                    f"    for pat in [{patterns_repr}]:\n"
                    f"        cnt = len(re.findall(pat, content))\n"
                    f"        if cnt:\n"
                    f"            sink_found = True\n"
                    ...
                    f"    print(f'Source not found: {{src}}')\n"
                    f"    sys.exit(1)\n"
                    f"# Step 2: 动态演示——os.path.join 拼接用户输入可逃逸基目录\n"
                    f"print('--- Dynamic verification: os.path.join traversal demo ---')\n"
                    f"demo_ok = False\n"
                    f"try:\n"
                    f"    base = '/workspace/app/data'\n"
                    f"    user_input = '../../etc/passwd'\n"
                    f"    joined = os.path.join(base, user_input)\n"
                    f"    normalized = os.path.normpath(joined)\n"
                    f"    base_norm = os.path.normpath(base)\n"
                    f"    escapes = normalized != base_norm and not normalized.startswith(base_norm + os.sep)\n"
                    f"    print(f'  join({{base!r}}, {{user_input!r}}) -> {{normalized}} (escapes base: {{escapes}})')\n"
                    f"    if escapes:\n"
                    f"        demo_ok = True\n"
                    f"except Exception as e:\n"
                    f"    print(f'  path traversal demo error: {{type(e).__name__}}: {{e}}')\n"
                    f"# 判定：sink 在案 + 逃逸演示成功 → 演示性确认；仅 sink → 静态档；无 sink → 无确认标记\n"
                    f"if sink_found and demo_ok:\n"
                    f"    print('VULNERABILITY_CONFIRMED(STATIC): path traversal demo - os.path.join/user path concatenation escapes base dir (source-asserted sink, no data-flow to target)')\n"
                    f"elif sink_found:\n"
                    f"    print('VULNERABILITY_STATIC_ONLY: file/path sink present in source; verify user-controlled path reaches the sink')\n"
                    f"elif content:\n"
                    f"    print('NO_SINK: 目标源码未发现路径拼接 sink 关键词，演示性确认不成立')\n"
                    f"print('=== Verification Complete ===')\n"
                    f"POC_EOF\n"
                    f"python3 /tmp/poc_{index}.py"
                ),
            ...
            'hardcoded_secret': {
                'command': (
                    ...
                    f"        if matches:\n"
                    f"            secret_count += len(matches)\n"
                    f"            print(f'  Secret pattern found: {{len(matches)}} matches')\n"
                    f"else:\n"
                    f"    print(f'Source not found: {{src}}')\n"
                    f"    sys.exit(1)\n"
                    f"# 密钥类无法动态确认（源码在案即证据）：N>0 → 静态档标记；无匹配 → 无确认标记\n"
                    f"if secret_count > 0:\n"
                    f"    print(f'VULNERABILITY_STATIC_ONLY: hardcoded secret pattern present in source ({{secret_count}} matches); rotate the secret and move it to env/secret manager')\n"
                    f"elif content:\n"
                    f"    print('NO_SINK: 目标源码未发现硬编码密钥 pattern，演示性确认不成立')\n"
                    f"print('=== Verification Complete ===')\n"
                    f"POC_EOF\n"
                    f"python3 /tmp/poc_{index}.py"
                ),
            ...
            'deserialization': {
                'command': (
                    ...
                    f"    with open(src) as f: content = f.read()\n"
                    f"    print(f'Source: {{len(content)}} chars loaded')\n"
                    f"    # Step 1: 静态确认危险反序列化 sink（pattern 按目标语言分流）\n"
                    f"    for pat in [{patterns_repr}]:\n"
                    f"        cnt = len(re.findall(pat, content))\n"
                    f"        if cnt:\n"
                    f"            sink_found = True\n"
                    f"            print(f'  Unsafe call \"{{pat}}\": {{cnt}} occurrences')\n"
                    f"else:\n"
                    f"    print(f'Source not found: {{src}}')\n"
                    f"    sys.exit(1)\n"
                    f"# Step 2: 可达输入构造演示——恶意 pickle payload 反序列化即触发副作用\n"
                    f"print('--- Dynamic verification: pickle payload construction demo ---')\n"
                    f"safe_data = json.dumps({{'test': 'data'}})\n"
                    f"print(f'JSON safe alternative: {{json.loads(safe_data)}}')\n"
                    f"demo_ok = False\n"
                    f"try:\n"
                    f"    class _DeserializationDemo:\n"
                    f"        def __reduce__(self):\n"
                    f"            return (print, ('DESERIALIZATION_DEMO_TRIGGERED',))\n"
                    f"    pickle.loads(pickle.dumps(_DeserializationDemo()))\n"
                    f"    demo_ok = True\n"
                    f"    print('  pickle payload constructed and deserialized -> side effect triggered')\n"
                    f"except Exception as e:\n"
                    f"    print(f'  deserialization demo error: {{type(e).__name__}}: {{e}}')\n"
                    f"# 判定：危险 sink + 可达输入演示 → 静态档（目标数据流需人工确认）；无 sink → 无确认标记\n"
                    f"if sink_found and demo_ok:\n"
                    f"    print('VULNERABILITY_STATIC_ONLY: unsafe deserialization sink present and reachable-input payload demo constructed (no data-flow to target source)')\n"
                    f"elif sink_found:\n"
                    f"    print('VULNERABILITY_STATIC_ONLY: dangerous deserialization sink present; verify untrusted input reaches the load call')\n"
                    f"elif content:\n"
                    f"    print('NO_SINK: 目标源码未发现危险反序列化 sink，演示性确认不成立')\n"
                    f"print('=== Verification Complete ===')\n"
                    f"POC_EOF\n"
                    f"python3 /tmp/poc_{index}.py"
                ),
