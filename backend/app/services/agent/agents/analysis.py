...
ANALYSIS_SYSTEM_PROMPT = """你是蓝鉴的漏洞分析 Agent，一个**自主**的安全专家。
...
class AnalysisAgent(BaseAgent):
    ...
    def __init__(
        ...
        full_system_prompt = build_enhanced_prompt(
            ...
            include_coverage_matrix=False,     # Analysis 不做覆盖率判断
            include_control_driven=False,      # 控制驱动由 Orchestrator 管理
...
Final Answer:""",
    ...
    async def run(self, input_data: Dict[str, Any]) -> AgentResult:
        ...
        # H1-fix: Read previously found findings for dedup injection
        existing_findings = previous_results.get("findings", [])
        dedup_summary = ""
        if existing_findings:
            dedup_lines = []
            for ef in existing_findings:
                ef_title = str(ef.get("title", ""))[:60]
                ef_file = str(ef.get("file_path", ""))[:80]
                ef_type = str(ef.get("vulnerability_type", ""))
                ef_line = ef.get("line_start", "")
                dedup_lines.append(f"- [{ef_type}] {ef_title} ({ef_file}:{ef_line})")
            dedup_summary = "\n## \u26a0\ufe0f \u5df2\u6709\u53d1\u73b0\uff08\u7981\u6b62\u91cd\u590d\uff09\n\u4ee5\u4e0b\u53d1\u73b0\u5df2\u7ecf\u88ab\u62a5\u544a\uff0c\u4e0d\u8981\u518d\u6b21\u62a5\u544a\u76f8\u540c\u7684\u95ee\u9898\uff1a\n" + "\n".join(dedup_lines[:20])
            if len(dedup_lines) > 20:
                dedup_summary += f"\n- ... \u8fd8\u6709 {len(dedup_lines) - 20} \u4e2a\u5df2\u6709\u53d1\u73b0"
            logger.info(f"[{self.name}] Injecting {len(existing_findings)} existing findings for dedup")
...
请开始你的安全分析。首先读取高风险区域的文件，然后**立即**分析其中的安全问题（输出 Action）。"""
        ...
        # 注入搜索约束
        if search_constraints and (search_constraints.get("files_read") or search_constraints.get("grep_patterns")):
            constraint_text = "\n## ⚠️ 搜索约束（禁止重复）\n"
            files_read = search_constraints.get("files_read", [])
            if files_read:
                constraint_text += f"### 禁止重读的文件（共{len(files_read)}个）\n"
                for f in files_read[:30]:
                    constraint_text += f"- {f}\n"
                if len(files_read) > 30:
                    constraint_text += f"- ... 还有 {len(files_read) - 30} 个文件\n"
            grep_patterns = search_constraints.get("grep_patterns", [])
            if grep_patterns:
                constraint_text += f"### 禁止重复的搜索模式（共{len(grep_patterns)}个）\n"
                for p in grep_patterns[:20]:
                    constraint_text += f"- {p}\n"
                if len(grep_patterns) > 20:
                    constraint_text += f"- ... 还有 {len(grep_patterns) - 20} 个模式\n"
            initial_message += constraint_text + "\n"
        ...
        # ✅ P1-3: 按需加载漏洞知识模块注入对话上下文
        try:
            knowledge_loader = KnowledgeLoader()
            knowledge_modules = self._infer_knowledge_modules(config, cross_round_context)
            if knowledge_modules:
                for module_name in knowledge_modules[:5]:
                    try:
                        module_content = await knowledge_loader.load_module(module_name)
                        if module_content:
                            self._conversation_history.append({
                                "role": "user",
                                "content": f"## 📚 专业知识模块: {module_name}\n\n{module_content}\n\n请在分析时参考以上知识。",
                            })
                            logger.info(f"[{self.name}] Loaded knowledge module: {module_name}")
                    except Exception as e:
                        logger.warning(f"[{self.name}] Failed to load knowledge module {module_name}: {e}")
        except Exception as e:
            logger.warning(f"[{self.name}] KnowledgeLoader init failed: {e}")
...
Final Answer: {{"findings": [...], "summary": "..."}}"""
    ...
    def _infer_knowledge_modules(self, config: Dict[str, Any], cross_round_context: str = "") -> List[str]:
        ...
        for vuln_type in target_vulns:
            if vuln_type in vuln_module_map:
                modules.append(vuln_module_map[vuln_type])
        ...
        if cross_round_context:
            gap_module_map = {
                ...
                "D7": "weak_crypto",
                "D9": "race_condition",
                "D10": "command_injection",
            }
            for dim_prefix, module_name in gap_module_map.items():
                if dim_prefix in cross_round_context and "未覆盖" in cross_round_context:
                    if module_name not in modules:
                        modules.append(module_name)

        if not modules:
            modules = ["sql_injection", "auth_bypass", "xss"]
