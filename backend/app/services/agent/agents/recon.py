...
from ..prompts import TOOL_USAGE_GUIDE, build_enhanced_prompt

logger = logging.getLogger(__name__)


RECON_SYSTEM_PROMPT = """你是蓝鉴的侦察 Agent，负责收集和分析项目信息。
...
class ReconAgent(BaseAgent):
    ...
    def __init__(
        ...
        # 使用 build_enhanced_prompt 注入防幻觉规则和文件验证
        full_system_prompt = build_enhanced_prompt(
            base_prompt=RECON_SYSTEM_PROMPT,
...
Final Answer:""",
    ...
    def _create_recon_handoff(self, final_result: Dict[str, Any]) -> TaskHandoff:
        ...
        summary = f"完成项目侦察: "
        if languages:
            # 兼容 languages 为 dict 列表的情况
            lang_names = [item if isinstance(item, str) else str(item.get("name", item)) for item in languages]
            summary += f"语言={', '.join(lang_names[:3])}; "
        if frameworks:
            # 兼容 frameworks 为 dict 列表的情况
            fw_names = [item if isinstance(item, str) else str(item.get("name", item)) for item in frameworks]
            summary += f"框架={', '.join(fw_names[:3])}; "
