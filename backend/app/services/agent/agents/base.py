...
from ..agent_contract import inject_agent_contract
...
class _LLMCriticalStreamError(Exception):
    """Internal signal: critical LLM API error to record in the circuit breaker."""

    def __init__(self, error_type: str, user_message: str) -> None:
        super().__init__(f"{error_type}: {user_message}")
...
class BaseAgent(ABC):
    ...
    def _get_llm_rate_limiter(self):
        ...
        优先返回 task-scoped 限流器（按 task_id 隔离，RPM 来自任务快照）；
        无 task_id 时退化为全局 get_llm_rate_limiter()（用于独立测试等场景）。
        """
        if self.task_id:
            return get_task_llm_rate_limiter(self.task_id, self.llm_rate_per_minute or 60)
        from ..core.rate_limiter import get_llm_rate_limiter

        return get_llm_rate_limiter()
    ...
    def _check_token_budget_exceeded(self, budget: int | None = None) -> bool:
        """P1: 检查累计 token 是否超过预算（BaseAgent 通用，orchestrator/子 agent 复用）。

        Args:
            budget: 显式预算值（用于测试）；None 时从 get_agent_config() 读取。
        budget<=0 视为未启用门禁。orchestrator 聚合 _total_tokens + _sub_agent_total_tokens；
        子 agent 仅有 _total_tokens（_sub_agent_total_tokens 不存在时按 0）。
        """
        if budget is None:
            from app.services.agent.config import get_agent_config
            budget = get_agent_config().token_budget
