...
from app.core.rbac import build_prompt_filter, get_subordinate_user_ids, UserRole
...
async def list_prompt_templates(
    ...
    # 数据范围过滤：系统模板 + 当前用户数据范围内的用户创建的模板
    subordinate_ids = []
    if current_user.role == UserRole.ADMIN:
        subordinate_ids = await get_subordinate_user_ids(db, current_user.id)

    prompt_filter = build_prompt_filter(current_user, subordinate_ids)
    if prompt_filter is not None:
        query = query.where(prompt_filter)
...
async def get_prompt_template(
    ...
    # 检查权限
    can_access = template.is_system
    if not can_access:
        if current_user.role == UserRole.SUPER_ADMIN:
            can_access = True
        elif current_user.role == UserRole.ADMIN:
            subordinate_ids = await get_subordinate_user_ids(db, current_user.id)
            if template.created_by == current_user.id or template.created_by in subordinate_ids:
                can_access = True
...
async def update_prompt_template(
    ...
    if template.is_system:
        ...
        if current_user.role != UserRole.SUPER_ADMIN:
            if template_in.is_active is not None:
                ...
                return PromptTemplateResponse(
                    ...
                    template_type=template.template_type,
                    content_zh=template.content_zh, content_en=template.content_en,
                    variables=variables,
                    is_default=template.is_default, is_system=template.is_system,
                    is_active=template.is_active, sort_order=template.sort_order,
                    created_by=template.created_by,
                    created_at=template.created_at, updated_at=template.updated_at,
                )
            else:
                raise HTTPException(status_code=403, detail="系统模板仅超级管理员可修改")
        else:
            # 超级管理员可以完整修改系统模板
        ...
        if template.created_by != current_user.id and current_user.role != UserRole.SUPER_ADMIN:
            ...
            if current_user.role == UserRole.ADMIN:
                subordinate_ids = await get_subordinate_user_ids(db, current_user.id)
                if template.created_by not in subordinate_ids:
...
async def delete_prompt_template(
    ...
    if template.created_by == current_user.id:
        ...
        subordinate_ids = await get_subordinate_user_ids(db, current_user.id)
        if template.created_by in subordinate_ids:
            can_delete = True
...
async def set_default_template(
    ...
    """设置默认模板（仅超级管理员）"""
    if current_user.role != UserRole.SUPER_ADMIN:
        raise HTTPException(status_code=403, detail="仅超级管理员可设置默认模板")
