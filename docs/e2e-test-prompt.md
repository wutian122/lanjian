...
| 项 | 值 |
|---|---|
| 服务器 A（arm64） | `10.129.7.87`，SSH 62222，前端 `http://10.129.7.87/`，backend `:8000` |
| 服务器 B（amd64） | `192.168.238.11`，SSH 22，前端 `http://192.168.238.11/`，backend `:8000` |
| 当前版本 | backend/frontend **v6.2.4**，sandbox **v6.1.0** |
| 管理员凭证 | 需先向用户索取当前有效的 admin 密码（勿硬编码） |
| 测试项目 | ZIP 导入优先（生产主流，如 nginx 源码 ZIP）；仓库导入可选 |
