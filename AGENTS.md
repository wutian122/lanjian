...
- 镜像来自 Docker Hub 组织 `wutian449`（lanjian-backend / lanjian-frontend / lanjian-sandbox），`v6.2.4`（backend/frontend）与 `v6.1.0`（sandbox）均为多架构（amd64 + arm64）；生产已锁 `v6.2.4` / `v6.1.0`，禁止 `:latest` 浮动。
