...
- `docs/` - 代码级流程分析（agent-execution-flow / audit-data-flow）+ 安全加固交付报告；另根目录有 `security-hardening-2026-08/`（安全加固变更规划工件）
...
| IMAGE_TAG | compose 显式锁 `v6.2.3`（backend/frontend），sandbox 锁 `v6.1.0`（`.env` 未设此变量） | compose 显式锁 `v6.2.3`（backend/frontend），sandbox 锁 `v6.1.0`（`.env` 残留惰性 `IMAGE_TAG=v5.0.0`，compose 已硬编码 tag 不受影响） |
...
- 镜像来自 Docker Hub 组织 `wutian449`（lanjian-backend / lanjian-frontend / lanjian-sandbox），`v6.2.4`（backend）与 `v6.2.3`（frontend）与 `v6.1.0`（sandbox）均为多架构（amd64 + arm64）；生产已锁 `v6.2.4` / `v6.2.3` / `v6.1.0`，禁止 `:latest` 浮动。
...
- **2026-08-27 v6.2.4 已部署**：两台服务器（A/B）backend 已升级到 `v6.2.4`（**frontend 保持 `v6.2.3`**、**sandbox 保持 `v6.1.0`**）。内容为任务临时目录清理根治（REQ-CLEAN-1/2/3：任务终态 finally 清理 `/tmp/lanjian/<task_id>`、reverify 兼容已清理目录（ZIP 重解压/仓库 409）、delete 任务兜底清理）——根因是服务器 A 的 `/tmp` 为 16G tmpfs，任务目录从不清理致 46 个目录累积 13G 塞满，新任务 `[Errno 28]` 失败并连带 db/redis 因 docker exec 写 `/tmp/runc-process*` 失败而误报 unhealthy（清理后自动恢复）。生产回归 PASS：任务 c7b646d7 终态后目录被清理、`/tmp` 保持 1%、db/redis healthy；Hub 多架构 push 因代理对 arm64 大层限速极慢（B amd64 已成功）。
