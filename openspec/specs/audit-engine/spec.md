...
Orchestrator 所有覆盖率放行分支（5 次拦截放行、analysis 重复调度放行、主迭代耗尽放行）SHALL 在 `coverage_bypass_info` 中携带 `gaps`、`block_count`、`reason`、`covered_count`、`total_dimensions`，并通过 `result.metadata` 传到完成回调，使 `COMPLETED_WITH_GAPS` 状态可追溯。
...
`SandboxManager.execute_tool_command` SHALL 将项目目录挂载到 `/workspace/src`（与 `execute_with_files` 一致），working_dir 设为 `/tmp`。`execute_python` SHALL 将 PoC 脚本写入 `/tmp/__poc.py`（tmpfs 可写）。`_command_needs_project_mount` SHALL 仅对 heredoc（`<<`）写入命令返回 False（不挂载），对含 `/workspace/` 路径引用的命令 SHALL 返回 True（需挂载项目以读取文件）。
