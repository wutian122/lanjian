...
export default function CreateAgentTaskDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="!w-[min(90vw,520px)] !max-w-none max-h-[85vh] flex flex-col p-0 gap-0">
        {/* Header */}
        <DialogHeader className="px-5 py-4 border-b border-border flex-shrink-0">
          <DialogTitle className="flex items-center gap-3 text-foreground">
            ...
            <div>
              <span className="text-base font-semibold">新建 Agent 审计</span>
            ...
            <ScrollArea className="h-[200px] border border-border rounded bg-muted/50">
              {loadingProjects ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground font-mono">
                  ...
                  <span className="text-sm">{searchTerm ? "未找到匹配" : "暂无项目"}</span>
                </div>
              ) : (
                <div className="p-1">
        ...
        {/* Footer */}
        <div className="flex-shrink-0 flex justify-end gap-3 px-5 py-4 border-t border-border">
          <Button
            variant="ghost"
