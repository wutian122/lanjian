...
export const api = {
  ...
  async getUserConfig(): Promise<{
    ...
    try {
      ...
      console.log('[API] getUserConfig 鎴愬姛:', {
      ...
      console.error('[API] getUserConfig 澶辫触:', e?.response?.status, e?.message);
      return null;
    }
  },
  ...
  async testSandboxNetwork(url: string): Promise<{
    ...
    const res = await apiClient.post('/config/test-sandbox-network', { url });
    return res.data;
  },

  async sandboxExec(command: string, timeout = 120, persistent = true): Promise<{
    success: boolean; stdout: string; stderr: string; exit_code: number; message: string;
  }> {
    const res = await apiClient.post('/config/sandbox-exec', { command, timeout, persistent });
    return res.data;
  },

  async getSandboxPackages(): Promise<{
    packages: { name: string; version: string }[]; total: number;
  }> {
