...
import { FileJson, FileText, Download, Loader2, AlertTriangle, FileSearch } from "lucide-react";
            ...
            <DialogContent className="sm:max-w-[600px]">
                ...
                <div className="py-4">
                    <RadioGroup
                        ...
                        className="space-y-4"
                    >
                        <div className="flex items-center space-x-3 p-4 border border-border rounded-lg bg-muted/50 cursor-pointer hover:bg-muted transition-colors">
                            ...
                            <Label htmlFor="json" className="flex items-center gap-3 cursor-pointer flex-1">
                                ...
                                <div>
                                    <div className="font-semibold text-foreground">JSON 格式</div>
                                    <div className="text-xs text-muted-foreground">结构化数据，适合程序处理和集成</div>
                                </div>
                            </Label>
                        </div>
                        <div className={`flex items-center space-x-3 p-4 border border-border rounded-lg bg-muted/50 ${isPdfDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:bg-muted transition-colors'}`}>
                            ...
                            <Label htmlFor="pdf" className={`flex items-center gap-3 flex-1 ${isPdfDisabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                                ...
                                <div>
                                    <div className="font-semibold text-foreground">PDF 格式</div>
                    ...
                    {/* 报告预览信息 */}
                    <div className="mt-6 border border-border rounded-lg bg-muted/50">
                        <div className="px-4 py-2 border-b border-border bg-muted flex items-center gap-2 rounded-t-lg">
                            <FileSearch className="w-4 h-4 text-primary" />
                            <h4 className="font-semibold text-foreground text-sm">报告内容预览</h4>
                        </div>
                        <div className="p-4 grid grid-cols-2 gap-3 text-xs">
                            <div className="flex items-center justify-between border-b border-border pb-2">
                                <span className="text-muted-foreground">编程语言:</span>
                                <span className="font-semibold text-sky-600">{language.toUpperCase()}</span>
                            </div>
                            <div className="flex items-center justify-between border-b border-border pb-2">
                                <span className="text-muted-foreground">质量评分:</span>
                                <span className="font-semibold text-emerald-600">{(analysisResult.quality_score ?? 0).toFixed(1)}/100</span>
                            </div>
                            <div className="flex items-center justify-between border-b border-border pb-2">
                                <span className="text-muted-foreground">发现问题:</span>
                                <span className="font-semibold text-amber-600">{analysisResult.issues?.length ?? 0}</span>
                            </div>
                            <div className="flex items-center justify-between border-b border-border pb-2">
                                <span className="text-muted-foreground">分析耗时:</span>
                                <span className="font-semibold text-foreground">{(analysisTime ?? 0).toFixed(2)}s</span>
                            </div>
                        </div>
                    </div>
                </div>
