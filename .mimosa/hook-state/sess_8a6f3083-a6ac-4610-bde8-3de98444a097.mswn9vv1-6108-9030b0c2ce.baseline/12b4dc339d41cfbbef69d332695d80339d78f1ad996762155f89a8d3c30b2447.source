...
class CoverageReport:
    ...
    def covered_count(self) -> int:
        # 修复：浅覆盖（keyword 命中）计入覆盖数，与 core/coverage 口径一致
        return sum(
            1 for status in self.statuses.values()
            if status in (CoverageStatus.COVERED, CoverageStatus.SHALLOW)
        )

    @property
    def deep_covered_count(self) -> int:
        return sum(1 for status in self.statuses.values() if status == CoverageStatus.COVERED)

    @property
    def required_covered(self) -> bool:
        # 修复：仅要求核心三角 D1/D2/D3 深度覆盖（原为全 10 维，过严）
        required_ids = {"D1", "D2", "D3"}
        return all(
            self.statuses.get(dimension.id) == CoverageStatus.COVERED
            for dimension in COVERAGE_DIMENSIONS
            if dimension.id in required_ids
        )

    @property
    def is_sufficient(self) -> bool:
        # 修复：covered(含浅覆盖) >= 6 且 D1/D2/D3 深度覆盖（原阈值 8 + 全维 required）
        return self.covered_count >= 6 and self.required_covered
