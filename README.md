...
[![Version](https://img.shields.io/badge/version-v6.0.3-brightgreen.svg)](https://github.com/wutian122/lanjian/releases)
...
**镜像版本控制**：`docker-compose.yml` 中的 `image` 显式锁版本到 `v6.0.3`（backend）/ `v6.0.2`（frontend）（生产已禁用 `:latest` 浮动 tag）。`v6.0.3`（backend）/`v6.0.2`（frontend）为 **多架构镜像**（`linux/amd64` + `linux/arm64`），`docker compose pull` 自动匹配宿主机架构，无需手动指定架构。
